...
function batchDemoResult(c, j) {
  ...
  return {
    ...
    subject: `【${c.role}のご実績に注目】${j.company || '弊社'}・${j.position}についてご相談`,
    mailBody: `突然のご連絡、誠に失礼いたします。${j.company || '弊社'}の採用担当と申します。\n${c.company}にて${c.role}としてご活躍中の貴殿、特に${sk0}のご経験を拝見し、ぜひ一度お話しさせていただきたいとご連絡いたしました。\n今回ご紹介する${j.position}では、${j.description.slice(0, 50)}…という業務をお任せしたいと考えています。\nまずは30分程度のカジュアルなお話しの機会をいただけますでしょうか。ご都合のよいお日時を2〜3つお知らせいただけますと幸いです。`
  };
}
